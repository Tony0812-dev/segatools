...
const struct dll_bind_sym carol_dll_syms[] = {
    {
        ...
        .sym = "carol_io_controlbd_start",
        .off = offsetof(struct carol_dll, controlbd_start),
    }, {
        .sym = "carol_io_controlbd_stop",
        .off = offsetof(struct carol_dll, controlbd_stop),
    }
};
...
HRESULT carol_dll_init(const struct carol_dll_config *cfg, HINSTANCE self)
{
    ...
    if (cfg->path[0] != L'\0') {
        ...
        if (owned == NULL) {
            ...
            dprintf("Wonderland Wars IO: Failed to load IO DLL: %lx: %S\n",
                    hr,
                    cfg->path);
        ...
        dprintf("Wonderland Wars IO: Using custom IO DLL: %S\n", cfg->path);
        src = owned;
    } else {
        owned = NULL;
        src = self;
    ...
    if (carol_dll.api_version >= 0x0200) {
        hr = E_NOTIMPL;
        dprintf("Wonderland Wars IO: Custom IO DLL implements an unsupported "
    ...
    if (FAILED(hr)) {
        if (src != self) {
            dprintf("Wonderland Wars IO: Custom IO DLL does not provide function "
